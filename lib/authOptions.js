  ...
  callbacks: {
    jwt: async ({ token, user, trigger, session }) => {
      if (user) {
        ...
        token.roleId = user.roleId ?? user.role;
        token.companyId = user.companyId;
        token.ownerId = user.ownerId ?? null;
        token.realOwnerId = user.ownerId ?? null;
        token.viewAsAdmin = false;
      }

      // Superadmin may preview Admin UI/API permissions via session.update({ viewAsAdmin })
      if (trigger === "update" && session && typeof session.viewAsAdmin === "boolean") {
        const realRole = Number(token.realRole ?? token.role);
        if (realRole === ROLE.SUPERADMIN) {
          token.viewAsAdmin = session.viewAsAdmin;
        }
      }

      const realRole = Number(token.realRole ?? token.role);
      if (!token.realRole) token.realRole = realRole;
      if (token.realOwnerId === undefined) {
        token.realOwnerId = token.ownerId ?? null;
      }

      if (realRole === ROLE.SUPERADMIN && token.viewAsAdmin) {
        // Effective Admin role for RBAC + owner scope (same as a real admin)
        token.role = ROLE.ADMIN;
        token.roleId = ROLE.ADMIN;
        token.ownerId =
          normalizeOwnerId(token.realOwnerId) || getDefaultOwnerId();
      } else {
        token.role = realRole;
        token.roleId = realRole;
        token.ownerId = token.realOwnerId ?? null;
        if (realRole !== ROLE.SUPERADMIN) {
          token.viewAsAdmin = false;
        }
      }
    ...
    session: async ({ session, token }) => {
      if (session.user) {
        ...
        session.user.viewAsAdmin = Boolean(token.viewAsAdmin);
